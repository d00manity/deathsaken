...
-- Instances to delete in Workspace
getgenv().workspaceTargets = {
    "GlitchParts"
}

-- Function to delete instances in Lighting
...
-- Function to delete instances in Workspace
...
-- Table of original hook and current hook
getgenv().baseHooks = {}
getgenv().currentHooks = {}

-- Create a common hook for remote by name
getgenv().initHook = function(targetRemote)
    getgenv().originalHookFunction = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if self == getgenv().MainRemote and getgenv().methodCalled == "FireServer" then
            -- Check if remote contains username
            if type(getgenv().parameters[1]) == "string" and getgenv().parameters[1]:find(game.Players.LocalPlayer.Name) and getgenv().parameters[1]:find(targetRemote) then
                -- If it is a vector sending remote, replace it with camera direction.
...
-- Turn off hook, restore original hook
...
-- If Error404 also detects the same animations, the above table can be used together.
-- If not, replace this board.
...
-- Replace the entire isAnimationMatching function with the below snippet
...
-- Function to store the original value of a player
...
-- When a new player enters, if the toggle is on, set it to false.
...
Miscs:CreateInput({
    Name = "Input FOV",
    PlaceholderText = "80", -- default suggested text
    RemoveTextAfterFocusLost = false,
    Flag = "FOVInput",
...
-- Initialize hook as soon as script loads
...
getgenv().toggleEnabled = false -- Toggle variable

-- Get animation name from AssetId
...
getgenv().setupCharacter = function(char)
    ...
    -- Replace animation when playing
...
local folderPath = "DEATHSAKEN/Assets"
if not isfolder("DEATHSAKEN") then makefolder("DEATHSAKEN") end
...
Miscs:CreateToggle({
    ...
    Callback = function(value)
        ...
        if getgenv().chatEnabled then
            ...
            -- Turn off chat window when toggle off
...
local roundtime
local positionSet = false -- To know if X scale has been adjusted
...
-- Function to get list of emotes from Purchased.Emotes
...
dropdownButton.Text = selectedEmote and ("Emote: "..selectedEmote) or "Select Emote"
